...
BACKUP_TEMPLATES = [cv2.imread("template7.jpg"), cv2.imread("template5.png")]
print(BACKUP_TEMPLATES[0])
# SCORE_CAP = 10.5 # neg inf to disable
SCORE_CAP = -float("inf")

WEIGHT_DYMANIC_TEMPLATE = 0
WEIGHT_ORGIONAL_TEMPLATE = 1

starting_frame = 18

weights = WEIGHT_DYMANIC_TEMPLATE+WEIGHT_ORGIONAL_TEMPLATE
...
def template_matching(img, template, step=None, last_pos=None):
    ...
    for meth in methods_choice:
        ...
        res = WEIGHT_DYMANIC_TEMPLATE * cv2.matchTemplate(img,template,method)
        res += WEIGHT_ORGIONAL_TEMPLATE * cv2.matchTemplate(img,STARTING_TEMPLATE,method)
        ...
        if last_pos is not None:
            max_loc, max_val = reduce_locations(last_pos, max_loc, res, max_val, speed ,verbose=True)

        if max_val < SCORE_CAP:
            print("backup is attempted: %f at %d" % (max_val, step))
            res2 = WEIGHT_ORGIONAL_TEMPLATE * cv2.matchTemplate(img,BACKUP_TEMPLATES[0],method)
            res2 += WEIGHT_DYMANIC_TEMPLATE * cv2.matchTemplate(img,template,method)
            min_val2, max_val2, min_loc2, max_loc2 = cv2.minMaxLoc(res2)
            if last_pos is not None:
                max_loc2, max_val2 = reduce_locations(last_pos, max_loc2, res2, max_val2, speed , verbose=True)
            if max_val2 > max_val:
                max_val = max_val2 
        ...
        if step in range(190,197):
            print(res.shape)
            cv2.imwrite('debug/%d.png' % step, (res/(weights))*255)
